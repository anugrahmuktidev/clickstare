<div class="w-full max-w-md space-y-5 bg-white p-6 rounded shadow">
  ...
  @error('jabatan') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
    ...
    <div class="block">
      ...
      <input id="register-password" type="{{ $showPassword ? 'text' : 'password' }}" wire:model="password"
        class="mt-1 w-full border rounded p-2">
      <div class="mt-2 flex items-center gap-2 text-sm text-gray-700">
        <input id="show-register-password" type="checkbox" wire:click="$toggle('showPassword')"
          @checked($showPassword)>
        <label for="show-register-password" class="select-none cursor-pointer">Tampilkan password</label>
      </div>
      @error('password') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
    </div>
    <div class="block">
      <label for="register-password-confirmation" class="text-sm">Konfirmasi Password</label>
      <input id="register-password-confirmation" type="{{ $showPasswordConfirmation ? 'text' : 'password' }}"
        wire:model="password_confirmation" class="mt-1 w-full border rounded p-2">
      <div class="mt-2 flex items-center gap-2 text-sm text-gray-700">
        <input id="show-register-password-confirmation" type="checkbox"
          wire:click="$toggle('showPasswordConfirmation')" @checked($showPasswordConfirmation)>
        <label for="show-register-password-confirmation" class="select-none cursor-pointer">Tampilkan konfirmasi
          password</label>
      </div>
    </div>
